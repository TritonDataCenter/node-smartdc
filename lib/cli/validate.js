...
function validateNetworkOptions(options) {
    ...
    if (options.hasOwnProperty('no_internet_nat')) {
        opts.internet_nat = false;
    }
    ...
    if (options.hasOwnProperty('routes')) {
        opts.routes = validateRoutes(options.routes);
    }
...
/**
 * A very simple validation that route objects contain string values.
 */
function validateRoutes(routes) {
    if (typeof (routes) !== 'object') {
        throw new TypeError('routes (object) required');
    }

    Object.keys(routes).forEach(function (net) {
        var route = routes[net];

        if (typeof (route) !== 'string') {
            throw new TypeError('route (string in object) required');
        }
    });
