...
var MasterOptions = {
    ...
    'state':
        'the state of the instance (i.e. running, stopped, etc...) (default: all)',
    'type':
        'filter instances by type (virtualmachine or smartmachine) (default: all)',
    'tombstone':
...
function buildDetailedUsageString(options) {
    assert.ok(options);

    var str = '\n\n';
    for (var k in MasterOptions) {
        if (options.hasOwnProperty(k)) {
