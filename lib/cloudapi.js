...
CloudAPI.prototype.getPackage = function(account, pkg, callback, noCache) {
  ...
  var self = this;
  return this._request(sprintf(PACKAGE, account, name), null, function(req) {
    return self._get(req, callback, noCache);
  });
};
...
CloudAPI.prototype.getDataset = function(account, dataset, callback, noCache) {
  ...
  var self = this;
  return this._request(sprintf(DATASET, account, name), null, function(req) {
    return self._get(req, callback, noCache);
  });
};
...
CloudAPI.prototype.createMachineSnapshot = function(account,
  ...
  var self = this;
  return this._request(sprintf(SNAPSHOTS, account, m), options, function(req) {
    return self._post(req, callback);
  });
};
 ...
 * @throws {TypeError} on bad input.
 */
CloudAPI.prototype.listMachineSnapshots = function(account, machine, callback) {
  if (typeof(machine) === 'function') {
    callback = machine;
    machine = account;
    account = this.account;
 ...
 * @throws {TypeError} on bad input.
 */
CloudAPI.prototype.addMachineTags = function(account, machine, tags, callback) {
  if (typeof(tags) === 'function') {
    callback = tags;
    tags = machine;
    machine = account;
...
CloudAPI.prototype.getInstValue = function(account, inst, callback) {
  ...
  var name = (typeof(inst) === 'object' ? inst.id : inst);
  var self = this;
  return this._request(sprintf(INST_RAW, account, name), null, function(req) {
    return self._get(req, callback, true);
  });
};
...
CloudAPI.prototype.getMachineUsage = function(account,
  ...
  console.error("getUsage in sdc-clients", account, machine, period)
  ...
  console.error("\n\n\ngetUsage path=%j", path)
  return this._request(path, null, function(req) {
    console.error("getUsage request=%j", req)
    return self._get(req, callback, noCache);
  });
};
...
CloudAPI.prototype._error = function(err) {
  ...
  if (err.httpCode >= 500 && err.details && err.details.body) {
    try {
      var response = JSON.parse(err.details.body);
    ...
    if (err.details && err.details.object && err.details.object.code)
      return _newError(err.details.object.code, err.details.object.message);
