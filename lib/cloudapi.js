...
function CloudAPI(options) {
    ...
    if (options.key) {
        ...
        // We allow dsa keys for smart-login, but not for node-smartdc
        if (/DSA/i.test(this.key)) {
            throw new TypeError('DSA keys are not supported by node-smartdc.');
        }
    } else {
        this.basicAuth = true;
    }
...
CloudAPI.prototype.createKey = function (account, options, callback) {
    ...
    /* BEGIN JSSTYLED */
    // if (!/^ssh-rsa.*/.test(options.key)) {
    //     throw new TypeError('Only RSA keys are supported');
    // }
    /* END JSSTYLED */
